...
class MusiciansCombinedExtractor(BaseFeaturesExtractor):
    def __init__(self, observation_space: gym.spaces.Dict, features_dim: int = 64):
        ...
        num_to_merge = 8 + 32 + num_attendee * 3 + num_musicians * 3 + 32 * 2
    ...
    def forward(self, observations) -> torch.Tensor:
        ...
        for key, extractor in self.extractors.items():
            if key == 'musicians_placed':
                ...
                att_hap = extractor(observations[key] / 1e6)
        ...
        encodings = torch.cat([placed_enc, instr_enc, mus_enc, att_enc, att_hap,
                               mus_att, mus_att_hap, att_att, cross_mus, cross_att],
                              dim=1)
