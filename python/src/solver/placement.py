...
def get_placements_hexagonal(width: float, height: float, min_distance: float = 10.):
    xs, ys = np.meshgrid(np.arange(min_distance, width - min_distance, min_distance),
                         np.arange(min_distance, height - min_distance, np.sqrt(3) * min_distance / 2))
    xs[::2, :] += min_distance / 2
    return xs, ys
